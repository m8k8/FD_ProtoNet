...
def local_cov_in_class(support_set,support_labels,nb_class,batchsize_per_task):
    support_set=support_set.view(batchsize_per_task,-1)
    support_labels=support_labels.view(batchsize_per_task,-1)
    batchsize_per_class=batchsize_per_task/nb_class
    index=torch.arange(support_set.shape[0])
    xx,yy=torch.meshgrid(index,index)
    sub=support_set[xx]-support_set[yy]
    norm_sub=torch.norm(sub,dim=2)
    a=torch.exp(-norm_sub*norm_sub*0.01)
    a=a.view(a.shape[0],a.shape[0],1)
    lindex=torch.arange(support_set.shape[0])
    lx,ly=torch.meshgrid(lindex,lindex)
    l=(support_labels[lx]==support_labels[ly])
    a=a*l
    a=a.view(a.shape[0],a.shape[1],1)
    a_sub=a*sub
    Sw=torch.einsum('ijk,ijl->kl',a_sub,sub)*0.5*(1.0/batchsize_per_class)
    return Sw

def local_cov_bet_class(support_set,support_labels,nb_class,batchsize_per_task):
    support_set=support_set.view(batchsize_per_task,-1)
    supprt_labels=support_labels.view(batchsize_per_task,-1)
    batchsize_per_class=batchsize_per_task/nb_class
    index=torch.arange(support_set.shape[0])
    xx,yy=torch.meshgrid(index,index)
    sub=support_set[xx]-support_set[yy]
    norm_sub=torch.norm(sub,dim=2)
    a1=torch.exp(-norm_sub*norm_sub*0.01)
    a1=a1.view(a1.shape[0],a1.shape[1],1)
    lindex=torch.arange(supprt_labels.shape[0])
    lx,ly=torch.meshgrid(lindex,lindex)
    l=(supprt_labels[lx]==supprt_labels[ly])
    a1=a1*l*(1.0/batchsize_per_task-1.0/batchsize_per_class)
    l2=(support_labels[lx]!=support_labels[ly])
    a2=l2*(1.0/batchsize_per_task)
    a2=a2.view(a2.shape[0],a2.shape[1],1)
    a=a1+a2
    a=a.view(a.shape[0],a.shape[1],1)
    a_sub=a*sub
    Sb=torch.einsum('ijk,ijl->kl',a_sub,sub)*0.5
    return Sb


def Fisher(support,support_labels,n_way,convertdim,dimension):
    batchsize=support.shape[0]
    n_support=support.shape[1]
    Fs=[]
    for i in range(batchsize):
        support_set=support[i]
        labels=support_labels[i]
        batchsize_per_task=n_support
        Sw=local_cov_in_class(support_set,labels,n_way,batchsize_per_task)
        Sb=local_cov_bet_class(support_set,labels,n_way,batchsize_per_task)
        Sb_Sw=Sb-Sw
        Sb_Sw=Sb_Sw.to('cpu').detach().numpy().copy()
        lam,v=np.linalg.eigh(Sb_Sw)
        lam=torch.from_numpy(lam.astype(np.float32)).clone()
        v=torch.from_numpy(v.astype(np.float32)).clone()
        v=v.to('cuda')
        lam=lam.to('cuda')
        eigen_id=torch.argsort(lam,descending=True)
        eigne_value=lam[eigen_id]
        eigen_vector=v[:,eigen_id]
        W=eigen_vector[:,:convertdim]
        W=W.view(dimension,convertdim)
        W=W/((torch.norm(W,dim=0)).view(1,convertdim))
        W=torch.t(W)
        Fs.append(W)
    return Fs
...
def ProtoNetHead(query, support, support_labels, n_way, n_shot, normalize=True):
    ...
    convertdim=n_support
    dimension=support.shape[2]


    Fs=Fisher(support,support_labels,n_way,convertdim,dimension)
    Fs=torch.stack(Fs)
    Fs_=Fs.expand(n_support,Fs.shape[0],Fs.shape[1],Fs.shape[2])
    Fs_=Fs_.permute(1,0,2,3)
    Fs_=Fs_.contiguous().view(Fs_.shape[0]*Fs_.shape[1],Fs_.shape[2],Fs_.shape[3])
    support=support.view(support.shape[0]*support.shape[1],support.shape[2],1)
    query=query.view(query.shape[0]*query.shape[1],query.shape[2],1)
    support=torch.matmul(Fs_,support)
    Fs_2=Fs.expand(n_query,Fs.shape[0],Fs.shape[1],Fs.shape[2])
    Fs_2=Fs_2.permute(1,0,2,3)
    Fs_2=Fs_2.contiguous().view(Fs_2.shape[0]*Fs_2.shape[1],Fs_2.shape[2],Fs_2.shape[3])
    query=torch.matmul(Fs_2,query)
    support=support.view(support.shape[0]//n_support,n_support,support.shape[1])
    query=query.view(query.shape[0]//n_query,n_query,query.shape[1])
